...
class Service(
    ...
    def delete_by_db_name(self, db_name: str) -> bool:
        ...
        Returns:
            bool: True if deletion was successful, False if datasource not found
        """
        db_config = self._dao.get_by_names(db_name)
        if db_config:
            self._db_summary_client.delete_db_profile(db_name)
            self._dao.delete({"id": db_config.id})
            return True
        return False
