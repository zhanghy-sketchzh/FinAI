...
class ConnectConfigDao(BaseDao):
    ...
    def get_db_list(self, db_name: Optional[str] = None, user_id: Optional[str] = None):
        ...
        if db_name and user_id:
            sql = f"SELECT * FROM connect_config where (user_id='{user_id}' or user_id='' or user_id IS NULL) and db_name='{db_name}' ORDER BY db_name"  # noqa
        elif user_id:
            sql = f"SELECT * FROM connect_config where user_id='{user_id}' or user_id='' or user_id IS NULL ORDER BY db_name"  # noqa
        elif db_name:
            sql = f"SELECT * FROM connect_config where  db_name='{db_name}' ORDER BY db_name"  # noqa
        else:
            sql = f"SELECT * FROM connect_config ORDER BY db_name"  # noqa
